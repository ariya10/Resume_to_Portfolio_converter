...
function Canvas2DPhysicsSpheres({ customization }: { customization: CustomizationOptions }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const primaryColor = customization.colors.primary || '#ec4899';
  const accentColor = customization.colors.accent || '#3b82f6';
  ...
  useEffect(() => {
    ...
    class Ball {
      ...
      draw() {
        ...
        grad.addColorStop(0.4, this.color + '38');
    ...
    const balls = [
      new Ball(width * 0.75, height * 0.3, 130, primaryColor),
      new Ball(width * 0.25, height * 0.7, 90, accentColor),
    ];
    ...
    const render = () => {
      ...
      const dist = Math.hypot(balls[0].x - balls[1].x, balls[0].y - balls[1].y);
      if (dist < 400) {
        const alpha = (400 - dist) / 400 * 0.15;
  ...
  return <canvas ref={canvasRef} className="absolute inset-0 w-full h-full block z-0 pointer-events-none opacity-50" />;
}

export default function PremiumMotion3dTemplate({ data, customization }: TemplateProps) {
  ...
  return (
    <div className="min-h-screen relative selection:text-white"
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden bg-[#0A0B0E]">
        <Canvas2DPhysicsSpheres customization={customization} />
      </div>
