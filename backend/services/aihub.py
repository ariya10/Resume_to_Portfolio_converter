...
PDF_ANALYSIS_MODEL = OLLAMA_MODEL
...
Read the attached PDF and extract the requested information. Follow the user's formatting instructions precisely (e.g., JSON or Markdown).""",
}
...
class AIHubService:
    ...
    def __init__(self):
        ...
        self.ollama_url = settings.app_ollama_url or OLLAMA_API_URL
        # Only initialize OpenAI client if configured for non-Ollama services
    ...
    def _extract_ollama_text(response_data: dict) -> str:
        """Extract assistant text from an Ollama JSON response."""
    ...
    async def gentxt(self, request: GenTxtRequest) -> GenTxtResponse:
        ...
        try:
            ...
            if use_ollama:
                ...
                content = self._extract_ollama_text(result_data)
                return GenTxtResponse(
                    content=content,
                    model=model_name,
                    usage=None,
...
"""
    ...
    def _extract_pdf_text(
        ...
        """Extract text from PDF pages and return as a single string."""
        try:
        ...
        try:
            ...
            start, end = AIHubService._resolve_pdf_page_range(total_pages=total_pages, page_start=page_start, page_end=page_end)
        ...
        return pdf_text, start, end, total_pages
    ...
    async def analyze_pdf(self, request: AnalyzePdfRequest) -> AnalyzePdfResponse:
        ...
        pdf_text, start, end, total_pages = self._extract_pdf_text(
            pdf_bytes=pdf_bytes,
